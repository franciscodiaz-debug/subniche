...
import { TradeInterestPickerPanel } from "@/components/trade-interests/shared/picker-panel"
...
function RightColumnContent({
  ...
  const appliedIds = trade.value.appliedInterestIds ?? []
  const excludedGlobalIds = trade.value.excludedGlobalIds ?? []

  return (
    <TradeInterestPickerPanel
      appliedIds={appliedIds}
      onChange={(ids) =>
        trade.onChange({ ...trade.value, appliedInterestIds: ids })
      }
      excludedGlobalIds={excludedGlobalIds}
      onExcludedChange={(ids) =>
        trade.onChange({ ...trade.value, excludedGlobalIds: ids })
      }
    />
  )
}
