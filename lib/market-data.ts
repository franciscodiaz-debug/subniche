...
export interface TradeableItemSummary {
  ...
  perfectCount: number
  /**
   * Optional variant / condition descriptor used by the expanded match
   * footer in TradeInterestCard. Keeps the per-row subtitle close to the
   * source of truth (each tradeable item) instead of leaking listing-detail
   * shape into the card. Omit if you don't want a subtitle line.
   */
  subtitle?: string
  /**
   * Optional asking / reference price for this tradeable item, in whole
   * dollars. Rendered right-aligned in the expanded match footer rows.
   */
  price?: number
}
...
export const myTradeableItems: TradeableItemSummary[] = [
  {
    ...
    matchCount: 5,
    perfectCount: 1,
    subtitle: "Olympic White — Excellent",
    price: 1799,
  },
  {
    id: "my-2",
...
export const tradeInboundInterests: (InboundInterest & {
  my_item_id: string
})[] = [
  {
    id: "ib-1",
  ...
  /* ---- Multi-match demo entries --------------------------------------------
   * These rows intentionally collide on `their_item.id` with earlier entries
   * so the deduplicated "All items" grid can prove out the "Matches X of
   * your items" rendering. Without them, every external listing would map
   * 1:1 to a single my_item and the multi-match branch would never trigger.
   *
   * Collisions:
   *   • `ib-1-item` (PRS Custom 24): matches my-1 (score 7.0, existing) AND
   *     my-2 (score 4.8, below) → 2 items.
   *   • `tm-1`      (Gibson Les Paul): perfect-matches my-1 (9.0, existing)
   *     AND inbound-matches my-2 (5.6, below) → 2 items across BOTH kinds,
   *     which also exercises the "perfect wins on tie" merge rule.
   */
  {
    id: "ib-6-multi",
    my_item_id: "my-2",
