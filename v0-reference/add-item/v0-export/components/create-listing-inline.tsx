...
const DEFAULT_SPEC_SCHEMA: { required: SpecField[]; optional: SpecField[] } = {
  required: [
...
const SPEC_SCHEMA: Record<string, { required: SpecField[]; optional: SpecField[] }> = {
  Guitars: {
    required: [
      { key: "brand", label: "Brand" },
      { key: "year", label: "Year" },
...
function AutoGrowTextarea({
  ...
  useAutoGrowTextarea(ref, value, true)
  return (
    <textarea
      ref={ref}
      value={value}
...
export function CreateListingInline({
  ...
  const handlePublish = () => {
    // Gate publish here (rather than disabling the button) so users get a visible
    // reason the action is blocked instead of a dead-feeling disabled state.
    if (!canPublish()) {
      setShowMissingError(true)
      return
    }
    setShowMissingError(false)
    console.log("[v0] Publishing item...", {
      images,
      category,
      subcategory,
      title,
      ...
      color,
      handedness,
      forSale: forSaleActive ? saleData : null,
      forTrade: forTradeActive ? tradeData : null,
      inCollection: inCollectionActive ? collectionData : null,
      wishlist: isWishlistActive ? wishlistData : null,
    })
  }
  ...
  return (
    <TooltipProvider>
      ...
      <div id="create-listing-top" className="px-4 py-6 md:px-6 lg:px-8">
        <div className="flex flex-col gap-4 mb-4">
          ...
          {/* Status picker */}
          <div data-onboarding="status" className="w-fit p-3 rounded-lg">
            <div className="contents">
              <div className="mb-2 flex items-center gap-2">
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="right" className="max-w-[280px] p-3">
                    <div className="text-xs space-y-1.5 leading-relaxed">
                      <p className="font-semibold">Items can have multiple states.</p>
                      <p className="text-muted-foreground">
                        An item can live in a collection and also be marked for sale or trade.
                      </p>
                    </div>
                  </TooltipContent>
                </Tooltip>
                {statusFieldsAdded &&
                  (() => {
                    // Color the status name to match the active chip tone so the
                    // acknowledgment visually threads back to the button tapped.
                    // Note: the dispatched label for collection is "Collection"
                    // (not "In Collection"), which is what we match on here.
                    const labelTone =
                      statusFieldsAdded === "For Sale"
                        ? "text-emerald-400"
                        : statusFieldsAdded === "Collection"
                          ? "text-primary"
                          : "text-foreground"
                    return (
                      <span className="text-xs text-foreground flex items-center gap-1 animate-in fade-in slide-in-from-left-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
                        <span className={cn("font-medium", labelTone)}>{statusFieldsAdded}</span>
                ...
                {tradeNoticeVisible && (
                  <span className="text-xs text-foreground flex items-center gap-1 animate-in fade-in slide-in-from-left-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
                    <span className="font-medium text-sky-400">For Trade</span>
                    <span>fields added – you&apos;ll set interests in the next step</span>
                  </span>
                )}
              </div>
              <StatusSelector
