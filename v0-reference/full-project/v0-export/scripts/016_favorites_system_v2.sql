-- Favorites System Migration v2
-- Tables for following (users, collections, items) and saved searches
...
DROP POLICY IF EXISTS "Users can view follows on their content" ON user_follows;

DROP POLICY IF EXISTS "Users can view own watchlist" ON watchlist_items;
DROP POLICY IF EXISTS "Users can insert own watchlist" ON watchlist_items;
DROP POLICY IF EXISTS "Users can update own watchlist" ON watchlist_items;
DROP POLICY IF EXISTS "Users can delete own watchlist" ON watchlist_items;
...
DROP POLICY IF EXISTS "Users can view own activity feed" ON activity_feed_events;
DROP POLICY IF EXISTS "Users can insert activity for own actions" ON activity_feed_events;
...
CREATE INDEX IF NOT EXISTS idx_user_follows_followed_user ON user_follows(followed_user_id) WHERE followed_user_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_user_follows_followed_collection ON user_follows(followed_collection_id) WHERE followed_collection_id IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_saved_searches_alerts ON saved_searches(alerts_enabled, alert_frequency) WHERE alerts_enabled = true;
...
CREATE INDEX IF NOT EXISTS idx_activity_feed_user ON activity_feed_events(for_user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_activity_feed_unread ON activity_feed_events(for_user_id, is_read) WHERE is_read = false;
CREATE INDEX IF NOT EXISTS idx_activity_feed_type ON activity_feed_events(event_type);
...
CREATE POLICY "Users can insert activity for own actions"
